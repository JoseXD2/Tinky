...
void mainImage()
{
    ...
    col.a = texture(iChannel0,vec2(x+uv.x+0.000,uv.y-0.002)).y+0.05;
    ...
    col.a += 0.05*texture(iChannel0,0.75*vec2(x+-0.022, -0.02)+vec2(uv.x+0.000,uv.y-0.002)).y;

    col = clamp(col*0.6+0.4*col*col*1.0,0.0,1.0);

    float vig = (0.0 + 1.0*16.0*uv.x*uv.y*(1.0-uv.x)*(1.0-uv.y));
	col *= vec4(pow(vig,0.3));

    col *= vec4(0.95,1.05,0.95, 1.);
	col *= 2.8;
	...
	col*=1.0-0.65*vec4(clamp((mod(fragCoord.x, 2.0)-1.0)*2.0,0.0,1.0));
